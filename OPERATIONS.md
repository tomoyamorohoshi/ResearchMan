...
**アイデア評価シグナル（分析パス2の入力に追加）**: `GET /api/idea-likes`・`GET /api/idea-trash`
（別実装のAPI。`FAVORITES_SYNC_TOKEN`を共用、レスポンス形状は`/api/favorites`と同じ
`{version:1, items:{[id]:{fav,ts}}}`。fav:trueがそれぞれ「いいね」「ゴミ箱在中」を表す）を取得し、
`data/ideas.json`と結合してパターン(pattern)別・参照先(refs先のcases.json tags/tech.json domains)別の
いいね/ゴミ箱分布、およびscores（`{discovery,surprise,conviction}`。2026-07-16以降に追加されたideaに
のみ存在しうる。2件未満または分散0の次元はPearson相関がnull=判定不能）との相関係数を計算する
（`scripts/lib/tuneup-stats.mjs`の`computeIdeaFeedbackStats`）。分析パス2「アイデア構造見直し」の
プロンプト（`scripts/lib/tuneup-prompts.mjs`の`buildPass2Prompt`。従来`biweekly-tuneup.mjs`に
インライン定義だったものをここへ切り出した）に「アイデア評価シグナル」として追加し、
いいね/ゴミ箱がともに0件のときはセクションごと省略する（trash/User事例と同じ縮退方針）。
endpointの導出は`deriveTrashEndpoint`と同じ流儀（favoritesのendpointから
`/api/favorites`→`/api/idea-likes`・`/api/idea-trash`を機械的に導出。
`~/.researchman-favsync.json`の`ideaLikesEndpoint`/`ideaTrashEndpoint`キーで明示指定も可）。
未設定・導出不可・エラー・503いずれも黙ってスキップし処理全体は継続する。

**切り口語彙の自動リフレッシュ**: `data/idea-angles.json`（DESIGN.md §6 idea。studio側の
`generateIdeaAnglesCli.ts`で生成する15〜25個の発想の型ライブラリ）を、`data/cases.json`件数が
前回生成時から**+50件以上**増えるたびに自動で再生成する（`scripts/lib/tuneup-angles.mjs`）。
  - 前回生成時のcases件数は新設の`data/idea-angles-meta.json`（`{caseCount, generatedAt}`）に記録する。
    `idea-angles.json`自体にメタを足さない（studio側の`validateIdeaAngles`が配列形状を前提にしており、
    メタ混入で壊れる可能性があるため）
  - 初回（メタ未設定）はいきなり再生成せず、現在のcases件数をベースラインとして記録するだけに留める
    （導入直後の週に無条件で高コストなClaude呼び出しが走らないようにするため）
  - 再生成は`studio/server/pipeline/generateIdeaAnglesCli.ts`を子プロセス実行するだけで、studio側の
    ロジック自体は無改変（studio/は編集対象外）。Windowsタスクスケジューラ環境はPATHが最小構成で
    `npx`解決に失敗するため、`scripts/lib/run-idea-layouts-precompute.mjs`と同じ
    「studioローカルインストール済みtsxを`process.execPath`経由で直接起動し、未インストール時のみ
    `npx`にフォールバック」方針を踏襲する
  - 再生成後、studio側の内部検証（`validateIdeaAngles`）を無条件に信用せず、チューンアップ側で
    独立にもう一段ガードレールをかける（`checkAnglesGuardrail`）: (a) 語彙数15〜25
    (b) 各`exemplarCaseIds`が`cases.json`の実在id (c) 旧語彙との入れ替わり率が80%以下
    （入れ替わり判定はid/label文字列ではなく`exemplarCaseIds`の重なり=Jaccard係数で行う。
    語彙は毎回ゼロから生成され文言が変わりうるが、exemplarCaseIdsは安定したid集合のため）。
    いずれか違反時は`git checkout`で`data/idea-angles.json`を旧語彙へ戻し、メタも更新しない
    （次回また+50件条件を満たせば再挑戦する）
  - 成功時はLINE報告に「切り口語彙を更新: `<旧数>`→`<新数>`語彙、新規: `<新規label一覧>`、
    削除: `<削除label一覧>`」を追記する（`diffAngleLabels`。判定基準はガードレールと同じ
    exemplarCaseIdsの重なり）
  - commit対象への追加は`data/idea-angles.json`・`data/idea-angles-meta.json`の2ファイル。
    `scripts/windows/run-job.mjs`（コミットフロー本体）は編集対象外のため、
    `biweekly-tuneup.mjs`が成功時に自らこの2ファイルを`git add`でステージしておき、
    `run-job.mjs`側が既存4ファイル（research-tuning.json等）を`git add`してcommitする際に
    自動的に相乗りさせる設計にした。**既知の制約**: この直後に他の理由（favorites取得失敗・
    ガードレール違反等）で`biweekly-tuneup.mjs`全体がexit 1すると、`run-job.mjs`のエラー分岐は
    commitを一切行わないため、ステージ済みの語彙変更はその週はcommitされず次回成功時まで
    持ち越される（変更が失われるわけではない）
  - コスト（Claude呼び出し）が大きいため`--dry-run`では呼ばない（`tuneup:dry`は
    「設定ファイル改訂フローの安価な検証」が目的のため）。**実発火の実行検証は行っていない**。
    「+50件条件の判定」「ガードレール（件数・実在id・入れ替わり率）」「メタファイルの読み書き」は
    `node scripts/smoke-tuneup-angles.mjs`で純関数として担保している
- 検証: `node scripts/smoke-tuneup-stats.mjs`（`computeIdeaFeedbackStats`・endpoint導出関数を追加）・
  `node scripts/smoke-tuneup-prompts.mjs`（`buildPass2Prompt`のアイデア評価シグナル反映・0件時省略を追加）・
  `node scripts/smoke-tuneup-angles.mjs`（新設。+50件条件・ガードレール・メタ読み書き・
  CLI呼び出しラッパーのコマンド組み立てを検証）
...
- **切り戻し**: LINE Developersコンソール、または
  ...
  常時セットアップが必要）。
- **応答の送信経路（2026-07-24〜）**: ウィザードの質問・確認・実行開始・バリデーション
  エラー等、対話への**即時応答**はreply API（`studio/server/line/reply.ts`）優先で送る。
  webhookイベントのreplyTokenを使うreply APIは無料枠（push 200通/月）を消費しない。
  replyToken失効時（期限切れ・使用済み等）はpushへ自動フォールバックする。一方、
  ジョブ完了・エラーの**事後通知**（数分〜数十分後）はreplyTokenがもう使えないため、
  引き続きpush（`push.ts`・`notify-line.mjs`）を使う。
...
| ideaseeds | 生成成功（種の配信） | critical（`--text-file`で本文そのものを配信するため。routine化するとダイジェスト要約でアイデアの種の全文が失われる） |
...
**月間見積もり**: digest最大31通/月 + ideaseeds（critical・種の配信）最大31通/月 +
Studio(LINE)発オンデマンドジョブ約20通/月 +
エラー系（critical: 収集エラー・push失敗・unverified等）約10通/月 ≈ 90通/月で、
既存の200通/月枠に十分収まる。**2026-07-24〜**: 上記の「Studio(LINE)発オンデマンドジョブ」
のうち対話中の即時応答（ウィザードの質問・確認等）はreply API経由に切り替わり無料枠を
消費しなくなったため、実際の消費はジョブ完了/エラーの事後通知分のみに減っている
（見積もり自体は変更していないため、実消費は上記より少ない安全側）。
...
### 瀬戸内国際芸術祭（芸術祭型アワードの実装例・2026-08-14）

`/awards/setouchi`。**部門を持たず開催年がコレクション単位になる**初のアワードで、
Cannesの「部門」位置に「開催年」が入る。2010/2013/2016/2019/2022/2025 の6コレクション・86件。

- `src/lib/awards.ts` の `isYearOnlyOrg()` がこの分岐。該当orgでは `parseCollection` が
  `category = 開催年` を返し、`toSlug`/`label` が年の二重化（`2010-2010`）を回避して
  `/awards/setouchi/2010` になる。**他orgの挙動は一切変えていない**
- award文字列は `瀬戸内国際芸術祭<年> <ラベル>` 形式。年が名称に密着し `\b` で拾えないため
  `parseCollection` に数字直接抽出のフォールバックがある（他orgの `\b` 判定は温存）

**参照リスト（`data/*-winners.json`）と専用監査スクリプトは意図的に作っていない。**
主催者が公募採用作品を招聘作品と区別して公表していないため、正解リストが世に存在せず、
作れば創作になるからである（調査の全過程と代替手順は `docs/AWARD_RESEARCH_SOP.md`
「分岐: 公式受賞者一覧が存在しないアワード（芸術祭型）」節）。**これは監査の作り忘れではない。**
pre-pushの4監査は瀬戸内データに対しても素通りではなく、サムネイル健全性監査は通常どおり効く。

確度ラベルは4段階。後から確証が出たらラベルを上げるだけで済む:

| ラベル | 根拠 | 件数 |
|---|---|---|
| `公募大賞` | 総括報告2013に明記（膨らんだ冒険／カンパニー・ディディエ・テロン） | 1 |
| `公募` | 公式アーカイブに「公募展」と明記（「連絡船」写真プロジェクト） | 1 |
| `公募候補` | 発表時期の差分照合による推定（2010年のみ実施済み） | 22 |
| `参加作品` | 上記以外 | 62 |

**データの再取得・追加手順**（公式アーカイブは作品ごとの詳細ページを持たない一覧のみ）:

- 一覧URL: `https://setouchi-artfest.jp/archive/artworks-artists?year=<年>&page=<n>`
  （12件/ページ。カードは `list-thumb-item` クラスで、画像・エリア・作品番号・作品名・作家名を持つ）
- **curlはUser-Agent必須**（無指定だと403）。PDFも同様
- サムネイルは `/files/ArchiveArtworkImages/<uuid>/value01/<hash>_m.jpg` が作品ごとに固有で、
  **既存サムネイルと重複しない**。そのため `audit-thumbnails.mjs` の重複検出を無改変で通る
  （ページのog:imageはサイト共通画像なので、そちらを使うと重複FAILする。必ずカード内の画像を使う）
- 公式アーカイブの総数は 2010:95 / 2013:237 / 2016:216 / 2019:221 / 2022:213 / 2025:280（計1262）。
  スクレイプ実績は計1259件（2016で2件・2019で1件、同一 作品番号+作品名+作家名 の重複により取りこぼす）
- 本文は**検証済みの事実のみ**で書く方針。記録の乏しい作品に推測を書かない（結果として
  記述の厚みは不均一になる。これは仕様）。会期・来場者数は総括報告PDF
  （`https://setouchi-artfest.jp/assets/files/about/archive/report<年>.pdf`）で確認できる
