...
| パス | 役割 | 注意 |
|---|---|---|
| `.last-research-run.txt` / `.last-tech-research-run.txt` / `.last-idea-seeds-run.txt` / `.last-tuneup-run.txt`（リポジトリ直下・gitignored） | 前回実行時刻。ゲートの判定材料（tuneupは毎月1日・15日08:30） | 消すと次の該当タイミングで即実行される（手動トリガとして使える） |
...
| `~/Library/Logs/researchman-auto.log` / `-tech.log` / `-ideas.log` / `-tuneup.log` | パイプライン全ログ | 期限前スキップ（exit 3）はログを出さない仕様。**0バイトでも異常ではない**。5MB超で`.log.1`へ自動ローテ（2026-07-04追加） |
