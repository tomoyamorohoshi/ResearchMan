...
- **サムネイル**（ローカル保存・照合検証つき、Case Studyと同基準）
- **技術の概要**: 1〜2行
- **技術のポイント**: 大学生にもわかるライトな文章で「何がすごいか」+ **「何が作れそうか」のヒント**を1段落
...
- **Step 0（パイロット）**: 受領済みブクマ24件を最優先候補として厳選20件でバックフィル
  → ユーザーが品質・カードデザインを確認 → OKなら半年分に拡充
- **Step 1（日次）**: 24hゲート（run-if-due方式の別ゲート）で日次収集。
  検証パイプライン（リンク死活→サムネ照合→記事化→デプロイ確認→通知）は既存資産を流用
