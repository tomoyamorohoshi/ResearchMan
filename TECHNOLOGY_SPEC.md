...
- **sources 語彙（出所）**: `X Bookmarks`（ユーザーのXブックマーク由来）/ `Batch Research`（一括リサーチ由来）/
  `Tech Radar`（日次自動収集 auto-research-tech 由来）。`build-tech-from-research.mjs` は `--source <label>`
  または候補レコードの `sources` で必ず指定する（未指定はエラー。全件 "X Bookmarks" 固定になる事故の再発防止）
