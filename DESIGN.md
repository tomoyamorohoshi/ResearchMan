...
- **idea タブ（テーマ駆動・事例の切り口を踏襲）**: テーマ(お題)＋縛り・文脈(任意)＋**切り口の源
  ...
  結果表示（自動反映済み）。各カードの中身は**RM本体のideaカード準拠**（ideas.json の
  title/pattern/seed/refs に対応）: **タイトル＋概要（「〜かも」文）はマスト**、切り口ラベル
  （pattern）を添え、**CASE:／TECH: の参考事例は refs にあれば表示**。`/ideas` へリンク。
  （2026-07-10 ユーザーFBで確定。モックv3の「angle＋seed一行」表示をこの書式で上書き）
