...
- 受賞者一覧から各作品の個別詳細ページへのリンクがある場合（運営団体の別ドメインのアーカイブサイト等を含む）、そのURLパターン（例: archive.example.org/prix/{ID}/）をstructureNoteに必ず記録する。後続フェーズは各作品の個別ページURLを必要とするため、この情報の有無が成否を分ける`;
}
...
[
  {
    ...
    "brand": "ブランド名",
    "agency": "制作会社（不明なら空文字）",
    "sourceUrl": "実際にWebFetchで確認した、その作品の個別詳細ページURL"
  }
]

## 厳守事項
- sourceUrlは実際にWebFetchで確認した実在URLのみ（記憶からの組み立て禁止）
- sourceUrlは作品ごとの個別詳細ページを使う（受賞者一覧ページに各作品へのリンクがあれば必ず辿って個別ページのURLを確認する）。複数作品で同一URLを使い回さない（sourceUrlはリンク先確認とサムネイル取得に使われるため、一覧ページの使い回しは同一サムネイル重複として監査で必ず失敗する）。個別ページがどうしても存在しない作品に限り一覧ページURLを可とするが、その場合も同一URLの作品は1件までとする
- 公式ソースで確認できない受賞者は含めない（トレード記事のみでの確定は禁止）
- 部門「${category}」に該当しない受賞者は含めない
- 該当する受賞者が1件も無ければ空配列 [] を返す
- ただし空配列を返してよいのは「公式ソース上に該当部門の受賞者が本当に掲載されていない」場合のみ。一覧ページに受賞者が掲載されているのに個別ページURLが分からない場合は、一覧ページHTML内のリンク（作品タイトルに張られたhref。運営団体の別ドメインのアーカイブサイトへのリンクを含む）を精査して対応する詳細URLを特定し直すこと。受賞者が掲載されているのに空配列を返すのは誤りである`;
}
