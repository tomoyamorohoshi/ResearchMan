...
// ── H-5再発防止: リンク正規化を scripts/lib/norm-link.mjs に統一 ─────────────
// 従来のnormalizeLink（pure.ts内部）は末尾スラッシュ除去のみで、host小文字化・fragment除去を
// しないnorm-link.mjsより弱かった。Studio Researchはこの弱い版を使うため、既存
// https://example.com/foo に対し候補 https://Example.com/foo#top が一致せず二重掲載していた
// （addCasePure.tsは既にnorm-link.mjsを使っており、同一サーバ内で判定が食い違っていた）。

test("dedupeCandidates: 大文字hostの候補は既存リンク（小文字host）と重複判定される", () => {
  const existing = buildExistingCaseIndex([
    { id: "a-2020", title: "A", link: "https://example.com/foo" },
  ]);
  const kept = dedupeCandidates(
    [{ title: "Totally Different Title", year: "2026", link: "https://Example.com/foo" }],
    existing,
  );
  assert.equal(kept.length, 0);
});

test("dedupeCandidates: fragment付きの候補は既存リンク（fragment無し）と重複判定される", () => {
  const existing = buildExistingCaseIndex([
    { id: "a-2020", title: "A", link: "https://example.com/foo" },
  ]);
  const kept = dedupeCandidates(
    [{ title: "Totally Different Title", year: "2026", link: "https://example.com/foo#top" }],
    existing,
  );
  assert.equal(kept.length, 0);
});

test("dedupeCandidates: 大文字host＋fragment＋末尾スラッシュが重なった候補も重複判定される", () => {
  const existing = buildExistingCaseIndex([
    { id: "a-2020", title: "A", link: "https://example.com/foo" },
  ]);
  const kept = dedupeCandidates(
...
// ── パース不能リンク(normLinkが""を返す)の偽陽性重複回帰 ─────────────
// normLinkはスキーム無し等パース不能なURLに対し""を返す。旧実装は linkKey が
// 空文字であることを無視して常に重複判定に使っていたため、id・タイトルが
// 全く別物の候補でも2件目以降が seenLinks.has("") で誤って重複扱いされ、
// 正当な新規事例が誤ってドロップされていた。

test("dedupeCandidates: id・タイトルが異なりパース不能なリンクを持つ候補は両方残る", () => {
  const existing = buildExistingCaseIndex([]);
  const kept = dedupeCandidates(
    [
      { title: "First Unparseable Link Case", year: "2026", link: "www.a.com/x" },
      { title: "Second Unparseable Link Case", year: "2027", link: "www.b.com/y" },
    ],
    existing,
  );
...
test("dedupeCandidates: 回帰確認 id同一の候補は従来どおり1件に絞られる", () => {
  const existing = buildExistingCaseIndex([]);
  const kept = dedupeCandidates(
    [
      { title: "Same Id Case", year: "2026", link: "https://example.com/one" },
      { title: "Same Id Case", year: "2026", link: "https://example.com/two" },
    ],
    existing,
  );
  assert.equal(kept.length, 1);
...
test("dedupeCandidates: 回帰確認 正規化タイトル同一の候補は従来どおり1件に絞られる", () => {
  const existing = buildExistingCaseIndex([]);
  const kept = dedupeCandidates(
    [
      { title: "The Blank Edition (2018)", year: "2018", link: "https://example.com/one" },
      { title: "the blank edition", year: "2018", link: "https://example.com/two" },
    ],
    existing,
  );
  assert.equal(kept.length, 1);
...
test("dedupeCandidates: 回帰確認 正常なhttpsリンク同一の候補は従来どおり1件に絞られる", () => {
  const existing = buildExistingCaseIndex([]);
  const kept = dedupeCandidates(
    [
      { title: "Different Title One", year: "2026", link: "https://example.com/same-path" },
      { title: "Different Title Two", year: "2027", link: "https://example.com/same-path" },
    ],
    existing,
  );
  assert.equal(kept.length, 1);
