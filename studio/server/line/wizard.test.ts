...
test("idle: 両方調べて・技術調べてもショートカットとして判定される", () => {
  const a = stepWizard(null, "両方調べて AR広告", NOW, USER);
  assert.equal(a.kind, "needsStructure");
  if (a.kind === "needsStructure") assert.equal(a.requestKind, "両方");
...
test("menu: 番号でkind確定しaway_themeへ", () => {
  ...
  const r = expectReply(outcome);
  assert.equal(r.pending?.state, "await_theme");
  assert.equal(r.pending?.kind, "両方");
});
