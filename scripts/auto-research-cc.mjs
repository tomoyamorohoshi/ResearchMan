...
5. **1記事1件**: 1つの記事URLからはその記事の主題である最重要の1件のみを出す。まとめ記事から複数の話題を切り出して複数件にしない。
${retryNote}
## 既存事例（これらは出さない）
${existingTitles}
...
- award欄は**公式ソース（アワード公式サイト・公式発表）で確認できた受賞のみ**記載する。
  記事中の伝聞や「受賞したらしい」「ノミネートされた模様」等の推測は書かない。
...
async function main() {
  ...
  // 同一記事URLからの重複カード生成防止（2026-07-13。automatonまとめ記事等の事故を受けて追加）
  ...
  for (let round = 1; round <= MAX_ROUNDS && toAdd.length < TARGET_NEW; round++) {
    ...
    for (const cand of found) {
      ...
      if (lk && existingLinkKeys.has(lk)) {
        console.log(`スキップ（同一記事の重複）: ${cand.title} → ${cand.link}`);
        stats.dup++;
        if (!DRY_RUN) {
