...
#
# 注意（2026-07-22修正）: cmd.exe の /c は「コマンド文字列が引用符で始まり、かつ引用符の
# 数が2個ちょうど・実行ファイル名のみを囲む」場合以外、先頭1文字が引用符なら「先頭の引用符を
# 剥がし、文字列中の最後の引用符も剥がす」という古い互換挙動にフォールバックする（cmd /?
# 参照）。このスクリプトのように `"node.exe" "script.mjs" >> "log" 2>&1` のように引用符が
# 3組以上あると誤って壊れ、ログファイルすら作成されない不具合が実機検証で確認された。
# 対策: コマンド全体をもう一段引用符で包む（"" ... ""）。cmd の互換挙動により、先頭の
# 引用符1個と末尾の引用符1個だけが剥がされ、内側の引用符構造はそのまま残るため正しく動く
# （実機で /c "..." 単体パターンと "" ... "" パターンを比較検証済み）。
