// アイデアの「いいね」サーバ同期API（/ideas ページ機能追加）のスモークテスト。
...
// - GET認証（Bearer FAVORITES_SYNC_TOKEN。favorites/trash APIと共用のtoken）が401/503を正しく返すこと
...
//   npx tsx scripts/smoke-idea-likes-api.mjs
import { MAX_ITEMS } from "../src/lib/favoritesMerge.ts";

const BASE_URL = process.env.IDEA_LIKES_SMOKE_URL || "http://localhost:3457";
...
async function postIdeaLikes(body, { rawBody } = {}) {
  return fetch(`${BASE_URL}/api/idea-likes`, {
    method: "POST",
...
async function main() {
  ...
  // --- GET: FAVORITES_SYNC_TOKEN前提の認証（favorites/trashと共用） ---
  {
    const res = await getIdeaLikes({ Authorization: `Bearer ${SYNC_TOKEN}` });
    assert(res.status !== 401, `正しいBearerトークンは401にならない (got ${res.status})`);
    assert(res.status === 503, `Blob未設定時はGETも503を返す (got ${res.status})`);
  }
  {
    const res = await getIdeaLikes({ Authorization: "Bearer wrong-token" });
    assert(res.status === 401, `誤ったBearerトークンは401を返す (got ${res.status})`);
  }
  {
    const res = await getIdeaLikes();
  ...
  if (failures > 0) {
    ...
    console.log("\n全テストPASS: idea-likes API");
  }
}

main().catch((err) => {
  console.error("smoke-idea-likes-api failed to run:", err);
  process.exit(1);
});
