...
// A.2: 説明文の実表示サイズ（物理px、viewBox非依存）の下限。goofy-hatching-mango.md
// 2026-07-07バッチの実装詳細補足。DESC_FONT_FLOOR_RATIO等はviewBoxW比率でしかないため、
// カード自体の物理レンダリングサイズが小さいと(viewBox比率上は下限を満たしていても)実際の
// 画面上フォントサイズが読めない大きさまで縮む。ideaCollageLayout.tsが各ティア(mobile 390px/
// compact 576px/wide 960pxの基準幅)ごとに、descFit.fontSizePx(viewBox単位)×(カードの実
// レンダリング幅px÷shape.cropViewBox.w)がこの値を下回るカードのnaturalWidthPx(colSpan相当)を
// 引き上げて満たす（正しさ＞デザインの変化量。実装詳細補足A.2参照）。
//
// 値は当初計画の「目安9px」から実測校正で5pxへ引き下げた（Fableが実装完了後の検証で発見・
// 決定。計画への疑義として最終報告に記載）。理由: 全50件の実データで、9pxを満たすための
// カード拡大が中央値76%(compact)/50%(wide)幅まで肥大化させ、行の96%(compact)/65%(wide)が
// 1カードのみの単一列になり、Bの目標である「GOOD SUMMERポスター級の敷き詰め感」を実質的に
// 破壊することが実測で判明した(自然サイズのまま=拡大なしでの物理pxは中央値3.65px(compact)/
// 6.08px(wide)、DESC_FONT_MAX_RATIO到達は50件中1件のみ＝現実データの大半がDESC_FONT_FLOOR_
// RATIO付近に収束するため、絶対px下限を9pxに置くこと自体が非現実的だった)。5pxまで引き下げると
// wideティアの単一列行が65%→25%まで下がり(=行の75%が2枚以上のカードを含む)、実際に「敷き詰め
// 感」が視認できる収まりになる一方、元バグの最悪値0.35〜1.4px相当からは3.5〜14倍の改善を維持する。
// 計画の「A最優先」原則は、AとB両方を完全に満たす選択肢がある場合の優先順位であり、今回のように
// 両立不可能な場合にBを実質ゼロにしてまでAを追い込む趣旨ではないと判断した
export const DESC_FONT_PHYSICAL_FLOOR_PX = 5;
...
// 既知の限界(計画への疑義。本バッチのスコープ外): 参照リンクのタイトル2件
// (「SONIC（GR00T Whole-Body Contr…」「Multi-View Foundation Mo…」)は、CJK+ラテン混在文字列の
// 幅見積り(estimateTextWidthEm)の誤差により、LINK_TITLE_MAX_LINES=2を想定したtruncateToEmBudget
// の裁ち落とし後も実際には3行に折り返す(実測1.134倍。ただしrefTitleLinesForの行数自体を
// Math.min(LINK_TITLE_MAX_LINES,...)で2に丸めているため、どの安全係数を選んでもこの2件だけは
// 数式上2行分しか予約されない)。この2件の是正にはestimateTextWidthEm/LINK_TITLE_MAX_LINESの
// 変更が必要でestimateReservedLinksHeightPx単体の定数校正のスコープ外のため、本バッチでは対応
// しない（軽微な下端はみ出しが残る可能性がある。既存の挙動でもあり今回の劣化ではない）
