...
// 画像の同時ロード数を制限するシンプルなキュー。pendingはid付きで保持し、
// ON遷移時にビューポート内カードのidをキュー先頭へ優先させられるようにする
// （2-1: prioritizeAndWaitThumbnails参照）
const MAX_CONCURRENT_LOADS = 12;
let activeLoads = 0;
const pending: Array<{ id: string; start: () => void }> = [];

function runWithLimit<T>(id: string, task: () => Promise<T>): Promise<T> {
...
// 進行中（または完了済み）のロードをid単位で1本化するPromiseキャッシュ。
// createImageSprite()と warmThumbnailCache()/prioritizeAndWaitThumbnails() の両方が
// 同じidのロードを要求しても、実際のネットワーク取得は1回だけになる
const loadPromises = new Map<string, Promise<HTMLImageElement | null>>();

function ensureLoadStarted(c: Case): Promise<HTMLImageElement | null> {
  const existing = loadPromises.get(c.id);
  if (existing) return existing;
  const p = runWithLimit(c.id, () => loadThumbnail(c)).then((img) => {
    imageCache.set(c.id, img);
    return img;
  });
  loadPromises.set(c.id, p);
  return p;
}
 ...
 */
export function warmThumbnailCache(cases: Case[]): void {
  for (const c of cases) {
    if (imageCache.get(c.id) !== undefined) continue; // ロード済み(成功/失敗確定)はスキップ
    ensureLoadStarted(c);
  }
}
 ...
 */
export function prioritizeAndWaitThumbnails(cases: Case[], timeoutMs: number): Promise<void> {
  const ids = new Set(cases.map((c) => c.id));
  pending.sort((a, b) => (ids.has(a.id) ? 0 : 1) - (ids.has(b.id) ? 0 : 1));
  const promises = cases.map((c) => ensureLoadStarted(c));
  return Promise.race([
