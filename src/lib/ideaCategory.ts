// Ideas タブのカテゴライズ・並び替えロジック（DESIGN: goofy-hatching-mango.md）。
// data/ideas.json を即時importしない、pure/クライアント安全なモジュール。
// src/lib/ideas.ts はdata/ideas.jsonをモジュール先頭で即時importするため、クライアント
// コンポーネント（IdeasPoster.tsx/IdeaShapeCard.tsx）がそこから値をimportするとバンドルに
// 巨大JSONが引きずり込まれるリスクがある。型・定数・純粋関数だけをここへ切り出し、
// クライアント側はここだけをimportする。src/lib/ideas.ts は後方互換のためこのモジュールを
// re-exportしつつ、data/ideas.json由来の値(ideas/sortedIdeas)だけを引き続き提供する。
