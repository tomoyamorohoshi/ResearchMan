...
export default function GalleryClient({ cases, categories, years, regions, sources = [], tags = [], defaultSort = "added", awardContext }: Props) {
  ...
  const filtered = cases.filter((c) => {
    ...
    if (showTrashOnly) {
      if (!trashed.has(c.id)) return false;
    } else if (trashed.has(c.id)) {
      return false;
    }
  ...
  // タブ（リサーチオーダー／Radar）— データに1件以上あるものだけ件数付きで表示（trashedは除外）
  ...
  const favoriteCount = mounted
    ? [...favorites].filter((id) => !trashed.has(id)).length
    : 0;
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={() =>
              setShowFavoritesOnly((v) => {
                const next = !v;
                if (next) setShowTrashOnly(false); // Trashビューとは排他
                return next;
              })
            }
          ...
          <button
            onClick={() =>
              setShowTrashOnly((v) => {
                const next = !v;
                if (next) setShowFavoritesOnly(false); // Savedビューとは排他
                return next;
              })
            }
            className={`flex items-center gap-1 text-[10px] tracking-[0.2em] uppercase font-bold transition-colors ${
            ...
            <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-px bg-gray-300">
              {sorted.map((c) => (
                <CaseCard
                  ...
                  isTrashed={trashMounted && trashed.has(c.id)}
