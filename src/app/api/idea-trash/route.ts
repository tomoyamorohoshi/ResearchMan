// アイデアの「ゴミ箱」サーバ同期API（/ideas ページ機能追加）。
...
import { readIdeaTrashBlob, writeIdeaTrashBlob } from "@/lib/favoritesStore";

// 常に最新のBlobを読む必要があるため静的キャッシュ対象にしない
export const dynamic = "force-dynamic";

export const { POST, GET } = createSyncRouteHandlers({
  readBlob: readIdeaTrashBlob,
  writeBlob: writeIdeaTrashBlob,
  notConfiguredMessage: "idea trash sync not configured",
  logLabel: "api/idea-trash",
});
