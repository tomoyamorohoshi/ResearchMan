...
`GET /api/idea-trash`（ゴミ箱＝弱化シグナル）を追加した。`data/ideas.json`のpattern別・
参照先(refs先の事例タグ/技術domain)別のいいね/ゴミ箱分布、およびscores
（`discovery`/`surprise`/`conviction`。2026-07-16以降のideaにのみ存在しうる）との相関を
分析材料に反映する（詳細は`OPERATIONS.md`「アイデア評価シグナル・切り口語彙の自動リフレッシュ」節）。

あわせて、`data/idea-angles.json`（発想の型ライブラリ）を`cases.json`蓄積が+50件増えるたびに
自動リフレッシュする仕組みを追加した。本ファイル（RESEARCH_PLAN.md）の収集方針とは独立した
仕組みのため、詳細はOPERATIONS.mdに委ねる。

## 変更履歴

- 2026-07-08: 初版作成（バッチ2b実装時。まだチューンアップ未実行のため現状の要約のみ）
- 2026-07-16: アイデア評価シグナル（idea-likes/idea-trash）・切り口語彙自動リフレッシュを追加
  （本節は次回チューンアップで上書きされる前提。仕組みの記録はOPERATIONS.md参照）
