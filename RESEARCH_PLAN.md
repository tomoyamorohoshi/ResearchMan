...
## 現在の関心仮説（2026-08-31更新）

お気に入り事例61件（全1082件中）、ごみ箱31件、ユーザー追加35件の分布を確認した。前回（2026-08-24、
お気に入り59件・ごみ箱30件・ユーザー追加33件）からの増分はごみ箱1件・お気に入り2件・ユーザー追加2件と
小さく、タグ別お気に入り率・ごみ箱率の傾向は前回とほぼ同一だった。有意な方向転換は見られないため、
厳守事項の「有意な傾向が見えない場合は無理に変えず現状維持」に従い、今回は探索角度・レーン・X Radar
クエリを変更せず据え置く。

- **強く好まれるタグ**（お気に入り率が全体平均を大きく上回る、順不同）: Theme/Sustainability（12/59=
  20%）、Form/Stunt（12/75=16%）、Tech/Data（11/83=13%）、Tech/Audio（5/47=11%）、Theme/Retail
  （6/62=10%）、Tech/3D・Form/Website・Form/Print・Theme/Food（8〜9%台）。データ/音声を素材にした
  表現、サステナビリティ/ソーシャルグッド訴求のスタント・OOH形式への支持は前回から継続して強い。
- **弱いタグ**（お気に入り率が低い、かつごみ箱率が高い＝明確な低関心シグナル）: Tech/Game（ごみ箱31件中
  16件＝過半数）、Theme/Entertainment（ごみ箱17件）、Form/Event（ごみ箱14件）、Tech/XR（ごみ箱8件）、
  Form/App（ごみ箱11件）。いずれもRadar由来のゲーム新作・VRイベント告知・展示会告知に集中しており、
  この種のニュース速報への低関心シグナルは前回から変化なし。
- **ユーザー追加事例（35件、強い意図シグナル）は、引き続きデータ/音声駆動のジェネラティブ・レスポンシブな
  ...
  Forest Carbon・Philharmonie Luxembourg等）。今回追加された2件（Girlguiding Uniform Redesign・
  Australia's Biggest Hands）はソーシャルグッド／スタント／食領域で、既存の強化方針（サステナビリティ・
  スタント）と整合しており、方針変更の必要はない。
- Technology側のお気に入りは152件中4件のみで、前回から増加なし。ドメイン別の傾向を判断するには
  依然サンプル数が不足（HCI/MediaArt・Motion/Body・CreatorTools・AI/Agents・Spatial/3Dに1〜2件ずつ
  分散、GenVideo・Audio/Musicは0件）。tech.lanesとX Radarクエリは無理に変えず現状維持とする。

## Case Study（auto-research-cc.mjs）の探索角度 — 現行3ラウンド（変更なし）
...
1. **海外の広告・クリエイティブキャンペーン + AI×クリエイティブ**（最重要）:
   ...
   タイポグラフィ、サステナビリティ/ソーシャルグッド訴求のスタント・OOHを優先的に拾う
2. **テック・プロダクト・XR・ゲーム・音楽×テクノロジー**（広告キャンペーン以外中心）:
   ...
   広告キャンペーンは最大2件。ゲーム/XR/VR関連は最大1件（ごみ箱率が全タグ中トップ2のため。
   特にゲーム新作リリース・VRイベント告知は避ける）。新ツール/プロダクト/ライブ演出/
...
- 2026-08-24: お気に入り59件・ごみ箱30件・ユーザー追加33件の分析に基づき、cc.roundFociの
  ...
  現状維持
- 2026-08-31: お気に入り61件・ごみ箱31件・ユーザー追加35件で再分析。前回からの増分が小さく傾向に
  有意な変化がないため、research-tuning.json・x-radar-queries.jsonともに変更なし（現状維持）。
  数値のみ更新
